...
def thm_chat(thm:Theorem_Wrapper, examples, num_lean_compiles, model, output_folder, csv_file, temperature=.6):
  chat = lean_chat(thm, examples, model, output_folder)
  create_csv('theorems.csv')
  with Dojo(chat.ld_thm) as (dojo, s0):
    num_hints=0
    for num_hints in range(thm.num_tactics):
      message=chat.partial_proof(num_hints)
